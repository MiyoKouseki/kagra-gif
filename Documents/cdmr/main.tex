...
\usepackage{here}
\usepackage{comment}
\title{Length fluctuation of baseline}
\author{Koseki Miyo}
\begin{document}
\setcounter{tocdepth}{2}
\maketitle
\abstract{大型重力波望遠鏡の安定性にとって問題となる脈動帯域以下の基線長伸縮について述べている。第一世代のような基線長が短い検出器では、腕共振器の鏡はそれぞれ同相でうごくため逆相成分である基線長伸縮は十分小さく低減されていた。しかし第二世代になると基線長伸縮は十分に低減されず、また高フィネスで線幅の小さい腕共振器を必要とするため、共振器制御は不安定になってしまう。本文書では、脈動が等方的に到来するという仮定に基づいて、この基線長伸縮の低減効果をモデル化した。このモデルによれば、KAGRAのアームの岩盤はVirgoと比べて、およそ$0.2\, \mathrm{Hz}$にある Second Microseism による基線長伸縮を10倍低減できることがわかった。この結果は、地下検出器は地面振動雑音や Newtonian 雑音 の低減だけでなく、干渉計の安定性の点においても優れていることを示唆している。}

\abstract{Length fluctuation of the baseline makes laser interferometric gravitational-wave detectors to be unstable. This disturbance is caused by seismic noises such as microseisms or earth quake bellow a few 100 mHz. Such low frequency motions are not reduced sufficiently even using the viblation isolation systems. In this paper, 
}
