%% main
function bodesusplotcmpopt2(sys,inpv,outv,freq,varargin)
...
for n=1:nin/2
    ...
    if strcmp(varargin{2*n-1},'xlim')
        xlimarg=varargin{2*n}; fg_x=1;
    end
end
...
for n=1:nplt
sysp=sys{n};
nin{n} =strcmp(sysp.InputName,inpv);
nout{n}=strcmp(sysp.OutputName,outv);
if isempty(nin{n})||isempty(nout{n})
    error('bodesusplotopt:InputError',...
        ['No input/output name found: {',inpv,',',outv,'}'])
end
% TRANSFER FUNCTION
[mag{n},phs{n}] = mybode(sysp(nout{n},nin{n})*calib{n},freq);
end

% UNIT CHECK
if ~fg_u1; unit1=unittake(inpv); end;
if ~fg_u2; unit2=unittake(outv); end;
if  strcmp(unit1,'1')||strcmp(unit2,'1'); unit='';
else unit=['[',unit2,'/',unit1,']']; end

% VARARGIN SET
if ~fg_c; colorarg={'r-','b-','g-','c-','m-','y-','k-',...
        'r--','b--','g--','c--','m--','y--','k--'}; end;
