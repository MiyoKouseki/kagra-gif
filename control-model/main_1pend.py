...
if True:
    ...
    mag_lowpass,phase_lowpass,omega_lowpass = bode(lowpass,
                                                   np.logspace(-4,4,1e5),
                                                   label='lowpass',Plot=False)
    mag_highpass,phase_highpass,omega_highpass = bode(highpass,
                                                      np.logspace(-4,4,1e5),
                                                      label='highpass',Plot=False)
    mag_sumoftwo,phase_sumoftwo,omega_sumoftwo = bode(sumoftwo,
                                                      np.logspace(-4,4,1e5),
                                                      label='sumoftwo',Plot=False)
...
if True:
    ...
    mag_oltf,phase_oltf,omega_oltf = bode(oltf,np.logspace(-4,4,1e5),
                                          label='oltf',Plot=False)
    mag_seis2stage,phase_seis2stage,omega_seis2stage = bode(seis2stage,
                                                            np.logspace(-4,4,1e5),
                                                            label='seis2stage',
                                                            Plot=False)
    mag_servo,phase_servo,omega_servo = bode(servo,np.logspace(-4,4,1e5),
                                             label='servo',Plot=False)
...
if True:
    ...
    mag_Tr120,phase_Tr120,omega_Tr120 = bode(Tr120,
                                             np.logspace(-4,4,1e5),
                                             label='Tr120',Plot=False)
    ...
    mag_Tr120_u,phase_Tr120_u,omega_Tr120_u = bode(Tr120_u,
                                                   np.logspace(-4,4,1e5),
                                                   label='Tr120_u',Plot=False)
...
if asdplot:
    ...
    ax0.loglog(omega_seis2stage/(2.0*np.pi),rms(mag_seis2stage,df),
               'r--',label='Closed Loop(RMS)')    
    ...
    plt.savefig('img_noisebudget_PI.png')
...
if plotservo:
    ...
    ax0.loglog(omega_Pa/(2.0*np.pi),mag_Pa,'k--',label='Actuator Response',alpha=0.5)
