...
if __name__ == '__main__':
    ...
    for theta in np.arange(0,90,1):
        ...
        Yend_NS = mptypes.Timeseries(Yend_NS,fs=8,name='Yend_NS')
        Yend_UD = mptypes.Timeseries(Yend_UD,fs=8,name='Yend_UD')
        Yend_EW.get_psd(ave=16,plot=False)
        Yend_NS.get_psd(ave=16,plot=False)
        Yend_UD.get_psd(ave=16,plot=False)
        #mpplot.LogLogPlot([Yend_EW,Yend_NS,Yend_UD],lim=(None,[1e-11,1e-7]),filename='./{0:03d}'.format(theta))
        mpplot.LogLogPlot([Yend_EW,Yend_NS],lim=(None,[1e-11,1e-7]),filename='./Yend_{0:03d}'.format(theta))
