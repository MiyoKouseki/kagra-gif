...
\end{eqnarray}
となる。振幅が減衰しない平面波が通過しているので，ここで$P_{u1}=P_{u2}=P_s$とした。また，信号雑音比$\mathrm{SNR}$を$\mathrm{SNR}=\frac{P_s}{P_n}$と定義した。
さてノイズのない平面波を測定した場合，変位を$u(x,t)=e^{i(\omega{t}-kx)}$とすると，コヒーレンスは$\mathrm{coh} = e^{ikL}$となる。したがって，式(\ref{eq:eq_planenoisecoh})に代入すると，
\begin{eqnarray}
...
さしあたりSelfNoise$n$と地面振動$x$を知りたいので、以下のような2種類の測定を行い$x,\,b,\,n$を評価する。
\begin{itembox}[l]{測定}
  測定A  
  \begin{itemize}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=11.5cm]{./coh_seismo.png}
  \end{center}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=11.5cm]{./cmrr_xarm.png}
  \end{center}
