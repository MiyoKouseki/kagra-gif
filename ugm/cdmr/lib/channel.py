...
def get_seis_chname(start,end,place='EXV',axis='all'):
    ...
    if place=='EYV':
        try:
            chname = list(map(lambda x:x.replace('EXV','EYV'),chname))
        except:
            chname = list(map(lambda x:x.replace('EX1','EY1'),chname))
    elif place=='IXV':            
        try:
            chname = list(map(lambda x:x.replace('EXV','IXV'),chname))
        except:
            chname = list(map(lambda x:x.replace('EX1','EX1'),chname))        
    elif place=='EXV':
        pass
    else:
        raise ValueError('!')
